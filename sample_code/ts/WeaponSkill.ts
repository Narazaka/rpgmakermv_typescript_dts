...
(function() {
    ...
    //
    Game_Actor.prototype.attackSkillId = function(this: Game_Actor): number
    {
        let normalId: number = Game_BattlerBase.prototype.attackSkillId.call(this);
        if (this.hasNoWeapons())
        {
            return normalId;
        }
        let weapon: IDataWeapon = this.weapons()[0];  // at plural weapon, one's first skill.
        let id: string = weapon.meta.skill_id;
    ...
    //
    let _Scene_Battle_commandAttack: Function = Scene_Battle.prototype.commandAttack;
    Scene_Battle.prototype.commandAttack = function(): void
    {
        BattleManager.inputtingAction().setAttack();
        // normal attack weapon (or other single attack weapon)
        let action: Game_Action = BattleManager.inputtingAction();
